...
public class HttpClient {
	...
	static {
		defaultProxyHost = Application.environment.getProperty("http.proxy.host");
		defaultProxyPort = Integer.parseInt(Application.environment.getProperty("http.proxy.port"));
	...
	public String invoke(String method, String url, String body, Map<String, String> headers, boolean doLog) throws IOException {
		...
		if(post instanceof HttpEntityEnclosingRequestBase) {
			StringEntity entity = charset == null ? new StringEntity(body) : new StringEntity(body, charset);
	...
	public <T> T invokeForEntity(String method, String url, Object body, Map<String, String> headers, Class<T> clazz, boolean log) throws IOException {
		...
		return Json.fromJson(jsonMapper, response, clazz);
	}
			...
			if (body instanceof String) {
				...
				if (typeHeader == null) {
					...
					if (body instanceof Map) {
						...
						stringBody = serializeMap(Json.fromJson(jsonMapper, Json.toJson(body), Map.class));
					}
				} else {
